...
d_all <- read_delim("data/environment/somlit_std/110524.csv", delim = ";",
...
#Save it
#save(dl, file="output/dl.Rdata")


data_env_mean<- dl%>%select(-qcode)%>%group_by(date,name)%>%summarize(value=mean(value, na.rm=TRUE))

#iInspection plot for one variable
data_env_mean%>%filter(name=="T")%>%ggplot()+geom_point(aes(x=date, y=value))

env_mean<- data_env_mean

#Save it
#save(env_mean, file="output/env_mean.Rdata")
